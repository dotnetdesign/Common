...
namespace DotNetDesign.Common
{
    ...
    public static class Guard
    {
        ...
        /// <returns>Whether or not the argument passes the check.</returns>
        public static bool ArgumentNotNullOrEmpty(string argument, string argumentName, bool allowWhitespace = false,
                                            bool throwIfFalse = true)
        {
            using (Logger.Assembly.Scope())
            {
                Exception ex;
                if (allowWhitespace)
                {
                    ...
                    ex = new ArgumentNullException(string.Format("Argument {0} cannot be null, empty, or just contain whitespace.", argumentName));
                }
                else
                {
